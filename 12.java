...
public class note1234 {
	public static void main(String[] args) {
		double total = 0;// initialize value 
		double avg =0;
		int max =999999999,min =0;
		Scanner scan = new Scanner(System.in);//function scanner start
		System.out.println("start with the minimum.");
		min = user_input_int(scan,max,min);
		System.out.println("now the maximum");
		max = user_input_int(scan,max,min);
		System.out.println("you entereda min of" + min +"and a max of" + max);
		// telling the user the range
		scan.close();// scan function close
		...
		*/	
		...
		avg = total /20;
		System.out.println("the average number after 20 roll will be:"+ avg);
		}
